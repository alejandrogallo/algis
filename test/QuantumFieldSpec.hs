...
main = hspec $ do
  ...
  describe "Expanding operators" $ do
    ...
    it "Expands sums ((a+b)**2)+" $ do
      (dag . expand) ((a + b) ** 2) `shouldBe` Osum
        [(dag a) ** 2, (dag b) * (dag a), (dag a) * (dag b), (dag b) ** 2]
    ...
    it "Expands sums (a+b)*(c+d)*i" $ do
      expand ((a + b) * (c + d) * i)
        `shouldBe` Osum [a * c * i, a * d * i, b * c * i, b * d * i]
    it "Expands sums (a+b)*i*(c+d)" $ do
      expand ((a + b) * i * (c + d))
        `shouldBe` Osum [a * i * c, a * i * d, b * i * c, b * i * d]
    it "Expands sums i*(a+b)*(c+d)" $ do
      expand (i * (a + b) * (c + d))
        `shouldBe` Osum [i * a * c, i * a * d, i * b * c, i * b * d]
    it "Expands sums (a+b)*(c+d)*(i+j)" $ do
      expand ((a + b) * (c + d) * (i + j)) `shouldBe` Osum
        [ a * c * i
        , a * c * j
        , a * d * i
        , a * d * j
    ...
    it "wick" $ do
      ...
      contractedCombinations [a, b, i]
        `shouldBe` [ []
                   ...
                   , [[(Particle "a", 1), (Hole "i", 3)]]
                   , [[(Particle "b", 2), (Hole "i", 3)]]
                   ]
